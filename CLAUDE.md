...
- Multi-city foundation: Search/concierge uses the active trip destination, chat history is scoped per trip (`anchor_chat:<tripId>`), `/api/concierge` must not refuse non-Tokyo trips, and `/api/tabelog` sends destination-aware Google Places queries while skipping Tabelog behavior outside Japan. Remaining multi-city work: Discover/Today built-in content and older onboarding copy still have Tokyo assumptions.
