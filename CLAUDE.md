...
- The Today tab owns trip selection and creation. Anchors, scratchpad, itinerary, flights, inbox, and hotel details are per trip. Creating a trip copies only training data from the active trip (`prefs` and `taste`), so edits in the new trip do not flow back to the source trip. Trip normalization must preserve the stored destination exactly; never coerce blank/non-Japan trips to Tokyo.
